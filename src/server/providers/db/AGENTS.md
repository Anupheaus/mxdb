...
- `withConnectionDb.ts` — `withConnectionDb(target, delegate)` — scopes the ambient context to the **pooled, watched** `ServerDb` for a `ConnectionDbTarget` (the same instance routed client connections to that database get) and runs `delegate`. For server-side work that belongs to a routed/tenant database but has no connection of its own (e.g. a consumer's scheduled jobs): writes flow through that database's change stream, so `onAfter*` hooks and client sync fire once, and `onChange` subscriptions made inside observe that database. `startServer` registers the pool via `setConnectionDbPool`; calling before then throws `InternalError`
...
- **Collection creation on a brand-new database** — the constructor kicks off `#configure()` (create collection, `collMod` to enable change-stream pre/post images, index reconciliation) in the background, and the first writes may ask for the same missing collections at the same moment. `#getCollectionByName` therefore memoises the in-flight `createCollection` per name so every caller awaits the one creation; the name is only added to the shared `collectionNames` set once creation succeeds, a failed creation is dropped so a later caller retries, and `NamespaceExists` (another process created it since start-up) is treated as success. `#configure()` is never awaited, so its failures are caught and logged as `Failed to configure collection` (the Mongo error's name/message/code/codeName are attached — native errors otherwise serialise to `{}`) rather than escaping as unhandled rejections.
