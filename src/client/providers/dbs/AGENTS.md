...
- **Audit table key is `PRIMARY KEY (id, recordId)`.** The same audit entry id legitimately appears under several records (one branch ULID applied to every record in a server push). New databases are created with this key (`buildAuditTableDDL` in `db-worker/buildTableDDL.ts`); `Db` migrates legacy `id TEXT PRIMARY KEY` tables on open using the same DDL. Before this, brand-new databases used the legacy key until their first reopen, so a shared branch anchor failed with a UNIQUE constraint in the first session and that record's audit never reached disk.
